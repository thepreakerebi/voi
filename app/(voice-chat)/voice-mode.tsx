...
export default function VoiceMode({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <section className="w-full h-[calc(100vh-56px)] md:px-2 py-4 md:py-8">
      <section className="grid w-full h-full mt-20 grid-cols-1 md:grid-cols-2 gap-4">
        <section className="rounded-[1.5rem] w-full bg-card h-full md:p-8">
          <Captions userText={userCaption} aiText={aiCaption} />
        </section>
        <section className="rounded-[1.5rem] w-full bg-transparent md:bg-card h-full p-0 md:p-8">
